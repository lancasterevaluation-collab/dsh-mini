...
| 一键验收 | `node src/apps/verify.ts` | 20/20 通过（含 CLI、对话框、GUI 三项端到端） |

## 3.5 后续：这个入口长出了另外两种形态

第 11 步交付的是"一条命令跑一个任务"的 CLI。之后在同一个应用层上又长出两个入口 ——
**它们没有改 agent 一行代码**，因为界面消费的是第 7 步就存在的 `session/event`：

| 入口 | 文件 | 与 CLI 的差别 |
|---|---|---|
| 对话框 | `apps/repl.ts` | 常驻，**复用同一个会话** → 多轮上下文（几乎免费：日志是唯一真相） |
| Web GUI | `apps/web.ts` + `apps/web-ui.html` | HTTP + SSE，把同一份事件流渲染成浏览器界面 |

这次扩展暴露了两个值得记住的教训，都写在了对应文件头里：

1. **`await rl.question()` 写的交互式程序无法自动验收。** Node 的 readline 在非 TTY
   的 stdin 上会一次性读完并 emit `line` 事件，此时第二个 `question()` 永远不 settle，
   表现为 `Detected unsettled top-level await` 然后进程静默退出。改成 `line` 事件 + 串行队列后，
   管道输入与真人敲键走同一条代码路径 —— 于是 `verify.ts` 能自动跑它。
2. **前端绝不能用 `innerHTML` 拼内容。** 助手回答与工具输出都是外部数据，
   一次 `innerHTML` 就能让工具结果里的 `<script>` 变成真的脚本。全部用 `textContent`。

还有一个**没做成的事**，如实记录：本机环境下拿不到界面的自动截图
（宿主内置浏览器持续报 `stale_document`，系统 Chrome/Edge 的无头模式被沙箱拦住
`Failed to open named pipe server ... 拒绝访问`）。所以界面的**功能**有自动化验收
（`verify.ts` 起服务、探端点、关服务），但**视觉**只能靠人打开看。

## L4 仍未解决

| 缺陷 | 后果 | 修法 | 当初为什么这么选 |
|---|---|---|---|
| **没有流式输出**（三个界面都有） | 长任务里用户看不到任何进展，只能等 | provider 加 `chatStream`，界面边收边打印 | 第 1 步的 `Provider` 是单次返回；流式要改接口与所有实现，属于第 1 步的范围扩大 |
| **单会话** | 一个进程只能跑一条会话，多 agent 场景要自己造 factory | 用 `session/factory` 服务，CLI 加 `--concurrent` | 课程目标是"跑通"，并发会引入会话隔离与日志交错两个新问题 |
| **没有交互式审批** | `ask` 裁决只能靠 `approve` 白名单，人无法临时批准 | 给对话框加一个读 stdin 的 `Approver`（约 30 行） | 见 L1 的 1.3：审批是服务，CLI 不该知道它的形态 —— Web GUI 里更明显，界面根本没做审批弹窗 |
| **Web GUI 没有鉴权** | 任何能访问 `127.0.0.1:8787` 的本地进程都能驱动 agent | 加一个 token 参数写进 URL | 它是"本机开发者的界面"，不是可部署的服务；只绑 `127.0.0.1` 是当前的边界 |
| **界面不展示完整工具输出** | 卡片里只有第一行摘要，要看全文得去翻 `.sessions/*.jsonl` | 把完整内容随事件一起推，前端做展开 | 一次工具输出可能几十 KB，全推会让 SSE 变成文件传输 |
